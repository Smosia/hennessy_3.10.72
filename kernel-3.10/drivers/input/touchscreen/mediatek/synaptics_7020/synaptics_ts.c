...
static struct synaptics_ts_data *ts_hub;
...
static int synaptics_tpd_flag;
static int synaptics_tpd_keylock_flag;
...
int synaptics_ts_read(struct i2c_client *client, u8 reg, int num, u8 *buf)
{
	...
	if (i2c_transfer(client->adapter, msgs, 2) < 0) {
		...
		if (pr_err_ratelimit()) {
			SYNAPTICS_ERR_MSG("transfer error\n");
			pr_err("[jhee: synaptics_ts_read] transfer error\n");
		}
		return -EIO;
	} else
		return 0;
		...
		if (ts->pdata->use_irq)
			...
			hrtimer_start(&ts->timer,
				      ktime_set(0,
						...
						(ts->pdata->ic_booting_delay * 1000000)),
				      HRTIMER_MODE_REL);
	...
	case IC_CTRL_BASELINE:
		...
		case BASELINE_REBASE:
			...
			if (likely(ts->finger_dsc.id != 0)) {
				if (unlikely
				    (i2c_smbus_write_byte_data(client, FINGER_COMMAND_REG, 0x1) <
				     0)) {
					SYNAPTICS_ERR_MSG
	...
	case IC_CTRL_REPORT_MODE:
		...
		case 0:	/* continuous mode */
			if (unlikely
			    (i2c_smbus_write_byte_data
			     (client, TWO_D_REPORTING_MODE,
			      REPORT_BEYOND_CLIP | ABS_MODE | CONTINUOUS_REPORT_MODE) < 0)) {
		...
		case 1:	/* reduced mode */
			if (unlikely
			    (i2c_smbus_write_byte_data
			     (client, TWO_D_REPORTING_MODE,
			      REPORT_BEYOND_CLIP | ABS_MODE | REDUCED_REPORT_MODE) < 0)) {
				SYNAPTICS_ERR_MSG("TWO_D_REPORTING_MODE write fail\n");
				return -EIO;
			}
			break;
...
int ghost_detect_solution(void)
{
	...
	if (first_int_detection) {
		for (cnt = 0; cnt < ts->pdata->num_of_finger; cnt++) {
			if (ts->finger_prestate[cnt] == TOUCH_PRESSED) {
				SYNAPTICS_INFO_MSG
				    ("ghost detected within first input time 200ms\n");
				ghost_detection = 1;
			}
		}
	}
	...
	if (ts->pre_ts_data.total_num) {
		if (old_ts_data.total_num != ts->pre_ts_data.total_num) {
			if (old_ts_data.total_num <= ts->pre_ts_data.total_num) {
				for (id = 0; id < ts->pdata->num_of_finger; id++) {
					if (ts->finger_prestate[id] == TOUCH_PRESSED
			...
			if (id < ts->pdata->num_of_finger) {
				...
				if ((ts->pre_ts_data.pos_x[id] > 0)
				&& (ts->pre_ts_data.pos_x[id] < ts->pdata->x_max)
				&& (1 <= old_ts_data.total_num)
				&& (1 <= ts->pre_ts_data.total_num)
				&& (jitter_sub(old_pos_x, ts->pre_ts_data.pos_x[id]) <=
				10)
				&& (jitter_sub(old_pos_y, ts->pre_ts_data.pos_y[id]) <=
				10)) {
				...
				} else if ((ts->pre_ts_data.pos_x[id] > 0)
					   && (ts->pre_ts_data.pos_x[id] < ts->pdata->x_max)
					   && (old_ts_data.total_num == 0)
					   && (ts->pre_ts_data.total_num == 1)
					   &&
					   (jitter_sub(old_pos_x, ts->pre_ts_data.pos_x[id])
					    <= 10)
					   &&
					...
					if (chk_time_interval
					   ...
					   jitter_sub(old_ts_data.total_num,
						      ts->pre_ts_data.total_num)) {
					ghost_detection = 1;
				} else
					;	/* do not anything */
				...
				if (4 < finger_subtraction_check_count) {
					finger_subtraction_check_count = 0;
					SYNAPTICS_INFO_MSG
					    ("need_to_rebase finger_subtraction!!!\n");
		...
		if (force_continuous_mode) {
			...
			if (t_ex_debug[TIME_EX_CURR_INT_TIME].tv_sec -
				...
				if (synaptics_ts_ic_ctrl
				    (ts->client, IC_CTRL_REPORT_MODE, ts->pdata->report_mode) < 0) {
			...
			for (cnt = 0; cnt < MAX_NUM_OF_FINGER; cnt++) {
				if (ts->finger_prestate[cnt] == TOUCH_PRESSED) {
					if ((finger_oldstate[cnt] == TOUCH_PRESSED)
					    &&
					    (jitter_sub
					     (old_ts_data.pos_x[cnt],
					      ts->pre_ts_data.pos_x[cnt]) < 10)
					    &&
					    (jitter_sub
					     (old_ts_data.pos_y[cnt],
					      ts->pre_ts_data.pos_y[cnt]) < 10)) {
						long_press_check = 1;
					}
				}
			}
	...
	for (id = 0; id < ts->pdata->num_of_button; id++) {
		if (ts->button_prestate[id] == TOUCH_PRESSED
		    && button_oldstate[id] == TOUCH_RELEASED) {
			button_check = 1;
			break;
		}
	}
	...
	if ((ghost_detection == 1) && (ts->pre_ts_data.total_num == 0)
	    && (ts->pre_ts_data.palm == 0)) {
...
*/
	cur_ver = (int)obsolete(&ts->config_id[1], NULL, 10);
	fw_ver = (int)obsolete(&ts->fw_config_id[1], NULL, 10);
...
static ssize_t store_firmware(struct device *dev, struct device_attribute *attr, const char *buf,
			      size_t size)
{
	struct synaptics_ts_data *ts;
	char path[256] = { 0 };
...
static ssize_t store_log(struct device *dev, struct device_attribute *attr, const char *buf,
			 size_t size)
{
	unsigned int mask;
	int ret;

	ret = sscanf(buf, "%x", &mask);
	if (ret < 0)
		return size;
...
static ssize_t store_f54(struct device *dev, struct device_attribute *attr, const char *buf,
			 size_t size)
{
	int ret = 0;
...
{
	struct synaptics_ts_data *ts =
	    container_of(to_delayed_work(button_lock_work), struct synaptics_ts_data,
			 button_lock_work);
	int ret;
...
static void synaptics_ts_work_func(struct work_struct *work)
{
	...
	if (ts->ic_init) {
		/* read device status */
		ret =
		    synaptics_ts_read(ts->client, DEVICE_STATUS_REG, sizeof(unsigned char),
		...
		ret =
		    synaptics_ts_read(ts->client, INTERRUPT_STATUS_REG, sizeof(unsigned char),
				      (u8 *) &ts_reg_ctrl.interrupt_status_reg);
		if (ret < 0) {
		...
		if (likely(ts->button_dsc.id != 0)) {
			ret =
			    synaptics_ts_page_data_read(ts->client, BUTTON_PAGE, BUTTON_DATA_REG,
							sizeof(ts_reg_ctrl.button_data_reg),
							(u8 *) &ts_reg_ctrl.button_data_reg);
			if (ret < 0) {
		...
		ret =
		    synaptics_ts_read(ts->client, FINGER_STATE_REG,
				      sizeof(ts_reg_data) -
				      ((MAX_NUM_OF_FINGER -
					ts->pdata->num_of_finger) * NUM_OF_EACH_FINGER_DATA_REG),
				      (u8 *) &ts_reg_data.finger_state_reg[0]);
		if (ret < 0) {
			SYNAPTICS_ERR_MSG("FINGER_STATE_REG read fail===========\n\n");
			SYNAPTICS_ERR_MSG("num_of_finger= %d\n", ts->pdata->num_of_finger);
			SYNAPTICS_ERR_MSG("sizeof(ts_reg_data)= %d, ", sizeof(ts_reg_data));
			SYNAPTICS_ERR_MSG("(Maxnum-num)*NUM_OF_EACH_FINGER_DATA_REG=%d\n",
				((MAX_NUM_OF_FINGER -
			       ts->pdata->num_of_finger) * NUM_OF_EACH_FINGER_DATA_REG));
			SYNAPTICS_ERR_MSG("NUM_OF_EACH_FINGER_DATA_REG=%d\n\n",
		...
		ret =
		    synaptics_ts_read(ts->client, INTERRUPT_ENABLE_REG, sizeof(ts->curr_int_mask),
				      (u8 *) &ts->curr_int_mask);
		...
		if (ts_reg_ctrl.interrupt_status_reg & ts->int_status_reg_asb0_bit &&
			...
			for (f_counter = 0; f_counter < ts->pdata->num_of_finger; f_counter++) {
					...
					if (finger_order == 0)
						SYNAPTICS_DEBUG_MSG
						    ("Finger status register%d: 0x%x\n", reg_num,
						     ts_reg_data.finger_state_reg[reg_num]);
				}
				...
				if (((ts_reg_data.finger_state_reg[reg_num] >> (finger_order * 2))
					...
					if (((ts_reg_data.
					      finger_data[f_counter][REG_WY_WX] & 0xF0) >> 4)
					    > (ts_reg_data.
					       finger_data[f_counter][REG_WY_WX] & 0x0F)) {
						width_max =
						    (ts_reg_data.
						     finger_data[f_counter][REG_WY_WX] & 0xF0) >> 4;
						width_min =
					...
					curr_ts_data.pressure[f_counter] =
					    ts_reg_data.finger_data[f_counter][REG_Z];

					if ((ts->pdata->use_ghost_detection) && (curr_ts_data.pressure[f_counter] == 0))
							pressure_zero = 1;
				...
				for (b_counter = 0;
				     ...
				     b_counter++)
					if (ts->button_prestate[b_counter] ==
					    TOUCH_PRESSED) {
						input_report_key(ts->input_dev,
								 ts->pdata->
								 button[b_counter],
								 BUTTON_CANCLED);
					if (likely(synaptics_rmi4_i2c_debug_mask &
					...
					ret = i2c_smbus_write_byte_data(ts->client, INTERRUPT_ENABLE_REG,
						ts->curr_int_mask);
					}
					...
					old_ts_data.pos_x[f_counter] =
					    ts->pre_ts_data.pos_x[f_counter];
					old_ts_data.pos_y[f_counter] =
					    ts->pre_ts_data.pos_y[f_counter];
					old_ts_data.pressure[f_counter] =
					...
					if (synaptics_rmi4_i2c_debug_mask &
					    SYNAPTICS_RMI4_I2C_DEBUG_FINGER_STATUS)
						SYNAPTICS_INFO_MSG
						    ("[Touch] Finger%d (%d, %d)released\n",
						     f_counter, ts->pre_ts_data.pos_x[f_counter],
						     ts->pre_ts_data.pos_y[f_counter]);
		...
		if (likely(ts->button_dsc.id != 0)) {
			...
			if ((ts_reg_ctrl.interrupt_status_reg & ts->int_status_reg_button_bit) &&
				...
				for (b_counter = 0; b_counter < ts->pdata->num_of_button;
				     b_counter++) {
					if (((ts_reg_ctrl.button_data_reg >> b_counter) & 0x1) == 1
					    && (ts->button_prestate[b_counter] == TOUCH_RELEASED)) {
						button_oldstate[b_counter] =
						    ts->button_prestate[b_counter];
			 ...
			 */
					    ...
					    if (((ts_reg_ctrl.
						  button_data_reg >> b_counter) & 0x1) == 0
						&& (ts->button_prestate[b_counter] ==
						    TOUCH_PRESSED)) {
						button_oldstate[b_counter] =
						    ts->button_prestate[b_counter];
					    ...
					    report_enable)
							SYNAPTICS_INFO_MSG
							    ("Touch KEY%d(code:%d) is %s\n",
							     ...
							     ts->pdata->button[b_counter],
							     ts->
							     button_prestate[b_counter] ? "pressed"
							     : "released");
				...
				if (ts_reg_ctrl.button_data_reg == 0) {
					queue_delayed_work(synaptics_wq, &ts->button_lock_work,
							   msecs_to_jiffies(200));
				}
...
&& (synaptics_rmi4_i2c_debug_mask & SYNAPTICS_RMI4_I2C_DEBUG_BUTTON_HANDLE_TIME)) {
		...
		if (ts->pdata->use_ghost_detection) {
			...
			if (ret == NEED_TO_OUT)
				...
				release_all_ts_event();
				synaptics_ts_hard_reset(ts);
				synaptics_ts_ic_ctrl(ts->client, IC_CTRL_REPORT_MODE,
...
exit_work:
		...
		if (ts->pdata->use_irq != 0) {
			if (mt_get_gpio_in(ts->pdata->int_gpio) != 1
				 ...
				 */
				SYNAPTICS_ERR_MSG("WARNING - Safety: ");
				SYNAPTICS_ERR_MSG("Interrupt line isn't set high on time cause unexpected incident\n");
			}
		}
		...
		if (!ts->is_probed) {
			...
			ret =
			    synaptics_ts_read(ts->client, MANUFACTURER_ID_REG, 1,
					      &ts->manufcturer_id);
			if (ret < 0)
			...
			ret =
			    synaptics_ts_read(ts->client, FLASH_CONFIG_ID_REG,
					      sizeof(ts->config_id) - 1, &ts->config_id[0]);
			if (ret < 0)
				SYNAPTICS_ERR_MSG("Config ID read fail\n");
			SYNAPTICS_INFO_MSG("Config ID: %s\n", ts->config_id);

			ret =
			    synaptics_ts_read(ts->client, SENSOR_MAX_X_POS, sizeof(resolution),
					      resolution);
			if (ret < 0)
			...
			ret =
			    synaptics_ts_read(ts->client, SENSOR_MAX_Y_POS, sizeof(resolution),
					      resolution);
			if (ret < 0)
			...
			ret =
			    synaptics_ts_read(ts->client, FLASH_CONTROL_REG, sizeof(flash_control),
					      &flash_control);
			if (ret < 0)
				SYNAPTICS_ERR_MSG("FLASH_CONTROL_REG read fail\n");

			ret =
			    synaptics_ts_read(ts->client, DEVICE_STATUS_REG, sizeof(device_status),
					      &device_status);
			if (ret < 0)
				SYNAPTICS_ERR_MSG("DEVICE_STATUS_REG read fail\n");
		...
		ret =
		    i2c_smbus_write_byte_data(ts->client, DEVICE_CONTROL_REG,
		...
		ret =
		    i2c_smbus_write_byte_data(ts->client, INTERRUPT_ENABLE_REG,
					      (temp | ts->int_status_reg_asb0_bit | ts->
					       int_status_reg_button_bit));
		if (ret < 0)
		...
		ret =
		    i2c_smbus_write_byte_data(ts->client, TWO_D_REPORTING_MODE,
					      (ts->pdata->
					       report_mode | ABS_MODE | REPORT_BEYOND_CLIP));
		if (ret < 0)
			SYNAPTICS_ERR_MSG("TWO_D_REPORTING_MODE write fail\n");

		ret =
		    i2c_smbus_write_byte_data(ts->client, DELTA_X_THRESH_REG,
					      (u8) ts->pdata->delta_pos_threshold);
		if (ret < 0)
...
static int synaptics_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (likely(ts->pdata->use_irq && ts->pdata->irq_num)) {
		...
		mt_eint_registration(ts->pdata->irq_num, ts->pdata->irqflags,
				     synaptics_eint_interrupt_handler, 1);
		mt_eint_unmask(ts->pdata->irq_num);
	...
	if (unlikely(!ts->pdata->use_irq)) {
		...
		hrtimer_start(&ts->timer,
			      ktime_set(0,
					...
					(ts->pdata->ic_booting_delay * 1000000)), HRTIMER_MODE_REL);
	}
...
#ifdef TOUCH_I2C_USE_DMA
	I2CDMABuf_va =
	    (u8 *) dma_alloc_coherent(NULL, 4096, (dma_addr_t *) &I2CDMABuf_pa, GFP_KERNEL);
	if (!I2CDMABuf_va) {
