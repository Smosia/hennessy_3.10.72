...
{
	pr_err("F11_Query_Base = 0x%x\n", F11_Query_Base);
	pr_err("F11_Cmd_Base = 0x%x\n", F11_Cmd_Base);
	pr_err("F11_Ctrl_Base = 0x%x\n", F11_Ctrl_Base);
	pr_err("F11_Data_Base = 0x%x\n", F11_Data_Base);
	pr_err("F34_Query_Base = 0x%x\n", F34_Query_Base);
	pr_err("F34_Cmd_Base = 0x%x\n", F34_Cmd_Base);
	pr_err("F34_Ctrl_Base = 0x%x\n", F34_Ctrl_Base);
	pr_err("F34_Data_Base = 0x%x\n", F34_Data_Base);
	pr_err("F01_Query_Base = 0x%x\n", F01_Query_Base);
	pr_err("F01_Cmd_Base = 0x%x\n", F01_Cmd_Base);
	pr_err("F01_Ctrl_Base = 0x%x\n", F01_Ctrl_Base);
	pr_err("F01_Data_Base = 0x%x\n", F01_Data_Base);
	pr_err("F54_Query_Base = 0x%x\n", F54_Query_Base);
	pr_err("F54_Command_Base = 0x%x\n", F54_Command_Base);
	pr_err("F54_Control_Base = 0x%x\n", F54_Control_Base);
	pr_err("F54_Data_Base = 0x%x\n", F54_Data_Base);
	pr_err("F54_Data_LowIndex = 0x%x\n", F54_Data_LowIndex);
	pr_err("F54_Data_HighIndex = 0x%x\n", F54_Data_HighIndex);
	pr_err("F54_Data_Buffer = 0x%x\n", F54_Data_Buffer);
	pr_err("F54_CBCSettings = 0x%x\n", F54_CBCSettings);
#ifdef _DS4_3_0_
	pr_err("F54_PhysicalRx_Addr = 0x%x\n", F54_PhysicalRx_Addr);
#endif
	pr_err("F1A_Query_Base = 0x%x\n", F1A_Query_Base);
	pr_err("F1A_Command_Base = 0x%x\n", F1A_Command_Base);
	pr_err("F1A_Control_Base = 0x%x\n", F1A_Control_Base);
	pr_err("F1A_Data_Base = 0x%x\n", F1A_Data_Base);
}
...
void SYNA_ConstructRMI_F1A(void)
{
	...
	for (k = 0; k < 8; k++) {
		/* if ((temp = ButtonMapping[k * 2 + 1]) == 0xFF) */
		temp = ButtonMapping[k * 2 + 1];
		if (temp == 0xFF)
			break;
		else {
			for (i = 0; i < numberOfRx; i++)
				if (RxChannelUsed[i] == temp)
					break;
			...
			if (CheckButton[0][i] == 0) {
				...
				for (j = 0; j < numberOfTx; j++)
					if (TxChannelUsed[j] == ButtonMapping[k * 2])
						break;
				CheckButton[j][i] = 2;
			} else {
				for (j = 0; j < numberOfTx; j++)
...
/* For Button Delta Image Display */
		...
		/* if ((temp = ButtonMapping[k * 2]) == 0xFF) */
		temp = ButtonMapping[k * 2];
		if (temp == 0xFF)
			break;	/* Button TX */
		else {
			for (i = 0; i < MaxNumberTx; i++) {
				if (TxChannelUsed[i] == temp)
					break;
				ButtonTXUsed[k]++;
			}
		}
