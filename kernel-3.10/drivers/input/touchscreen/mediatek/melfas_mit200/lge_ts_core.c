 ...
 */
u32 touch_debug_mask_ = 0 | DEBUG_BASE_INFO | DEBUG_CONFIG | DEBUG_TRACE
| DEBUG_POWER | DEBUG_FW_UPGRADE | DEBUG_ABS_POINT;
...
{
	struct lge_touch_data *ts = container_of(timer, struct lge_touch_data, trigger_timer);
...
void trigger_baseline_state_machine(int usb_type)
{
	...
	if (touch_test_dev && touch_test_dev->pdata->role->ghost_detection_enable) {
		if (atomic_read(&touch_test_dev->curr_pwr_state) == POWER_ON) {
			if (usb_type == 0) {
		...
		if (trigger_baseline == 0) {
			trigger_baseline = 1;
			hrtimer_start(&ts->trigger_timer, ktime_set(0, MS_TO_NS(1000)),
				      HRTIMER_MODE_REL);
		}
	}
}

#define DEBUG_DELTA_TIME() (t_ex_debug[TIME_EX_CURR_PRESS_TIME].tv_sec \
-t_ex_debug[TIME_EX_PREV_PRESS_TIME].tv_sec)

#define DEBUG_INTERVAL_TIME(inc) get_time_interval(t_ex_debug[TIME_EX_CURR_PRESS_TIME].tv_usec + inc,\
t_ex_debug[TIME_EX_PREV_PRESS_TIME].tv_usec)

#define BT_PRESS_TIME() (t_ex_debug[TIME_EX_BUTTON_PRESS_END_TIME].tv_sec \
-t_ex_debug[TIME_EX_BUTTON_PRESS_START_TIME].tv_sec)

#define BT_PRESS_INTERVAL_TIME(inc) get_time_interval(t_ex_debug[TIME_EX_BUTTON_PRESS_END_TIME].tv_sec+inc \
-t_ex_debug[TIME_EX_BUTTON_PRESS_START_TIME].tv_sec)

/* Ghost Detection Solution */
static u8 resume_flag;
static void ghost_detect_update(struct lge_touch_data *ts)
{
	memcpy(&t_ex_debug[TIME_EX_PREV_PRESS_TIME],
	       &t_ex_debug[TIME_EX_CURR_PRESS_TIME], sizeof(struct timeval));
	do_gettimeofday(&t_ex_debug[TIME_EX_CURR_PRESS_TIME]);

	if (1 <= ts->ts_data.prev_total_num
	    && 1 <= ts->ts_data.total_num
	    && jitter_sub(ts_prev_finger_press_data.x_position,
			  ts->ts_data.curr_data[id].x_position) <=
	    10
	    && jitter_sub(ts_prev_finger_press_data.y_position,
			  ts->ts_data.curr_data[id].y_position) <= 10) {
		/* if time_interval between prev fingger pressed and curr
		   finger pressed is less than 50ms, we need to rebase touch ic. */
		if (DEBUG_DELTA_TIME() == 1 && DEBUG_INTERVAL_TIME(1000000) <= 50 * 1000) {
			ghost_detection = true;
			ghost_detection_count++;
		} else if (DEBUG_DELTA_TIME() == 0 && DEBUG_INTERVAL_TIME(0) <= 50 * 1000) {
			ghost_detection = true;
			ghost_detection_count++;
		}
	} else if (ts->ts_data.prev_total_num == 0
		   && ts->ts_data.total_num == 1
		   &&
		   jitter_sub(ts_prev_finger_press_data.x_position,
			      ts->ts_data.curr_data[id].x_position) <= 10
		   &&
		   jitter_sub(ts_prev_finger_press_data.y_position,
			      ts->ts_data.curr_data[id].y_position) <= 10) {
		/* if time_interval between prev fingger pressed and curr
	...
	memcpy(&ts_prev_finger_press_data,
	       &ts->ts_data.curr_data[id], sizeof(ts_prev_finger_press_data));
}
...
int ghost_detect_solution(struct lge_touch_data *ts)
{
	...
	if (ts_charger_plug) {
		if ((ts_role->ta_debouncing_finger_num <= ts->ts_data.total_num)
		    && (ta_debouncing_count < ts_role->ta_debouncing_count)) {
	...
	if ((ts->ts_data.state != TOUCH_ABS_LOCK) && (ts->ts_data.total_num)) {
		...
		if (ts->ts_data.prev_total_num != ts->ts_data.total_num) {
			if (ts->ts_data.prev_total_num <= ts->ts_data.total_num) {
				...
				if (DEBUG_DELTA_TIME() == 1 && DEBUG_INTERVAL_TIME(1000000) < 11 * 1000)
					)
					    finger_subtraction_check_count++;
				else if (DEBUG_DELTA_TIME() == 0
					 && DEBUG_INTERVAL_TIME(0) < 11 * 1000)
					finger_subtraction_check_count++;
				else
	...
	if (ts->ts_data.state != TOUCH_BUTTON_LOCK) {
		if (ts->work_sync_err_cnt > 0 && ts->ts_data.prev_button.state == BUTTON_RELEASED) {
			/* Do nothing */
		} else {
			...
			if (6 <= button_press_count) {
				if (BT_PRESS_TIME() == 1 && BT_PRESS_INTERVAL_TIME(1000000) <= 100 * 1000)
					) {
					TOUCH_INFO_MSG("need_to_rebase button zero\n");
					goto out_need_to_rebase;
				} else
				    if (BT_PRESS_TIME() == 0 && BT_PRESS_INTERVAL_TIME(0) <=
					100 * 1000)
					) {
					TOUCH_INFO_MSG("need_to_rebase button zero\n");
					goto out_need_to_rebase;
				...
				if (BT_PRESS_TIME() > 1)
					button_press_count = 0;
				else if (BT_PRESS_TIME() == 1	&& BT_PRESS_INTERVAL_TIME(1000000) >= 100 * 1000)
					button_press_count = 0;
				else if (BT_PRESS_TIME() == 0	&& BT_PRESS_INTERVAL_TIME(0) >= 100 * 1000)
					button_press_count = 0;
			}
		}
	}
		...
		for (id = 0; id < ts_caps->max_id; id++) {
			...
			if (ts->ts_data.curr_data[id].status == FINGER_PRESSED) {
				...
				if (unlikely(touch_debug_mask_ & DEBUG_ABS))
					TOUCH_INFO_MSG
					    ("hyj-press[%d] pos[%4d,%4d] w_m[%2d] w_n[%2d] w_o[%2d] p[%3d]\n",
					     ts_caps->is_id_supported ? id : 0,
					     ts->ts_data.curr_data[id].x_position,
					     ts->ts_data.curr_data[id].y_position,
					     ts_caps->is_width_supported ? ts->ts_data.
					     curr_data[id].width_major : 0,
					     ts_caps->is_width_supported ? ts->ts_data.
					     curr_data[id].width_minor : 0,
					     ts_caps->is_width_supported ? ts->ts_data.
					     curr_data[id].width_orientation : 0,
					     ts_caps->is_pressure_supported ? ts->ts_data.
					     curr_data[id].pressure : 0);
			} else {
				/* release handling */
				if (ts_role->key_type != TOUCH_SOFT_KEY
					...
					memset(&ts->ts_data.prev_data[id], 0x0,
					       sizeof(ts->ts_data.prev_data[id]));
...
static void release_all_ts_event(struct lge_touch_data *ts)
{
	...
	if (ts_role->key_type == TOUCH_HARD_KEY) {
		...
		if (ts->ts_data.prev_button.state == BUTTON_PRESSED) {
			input_report_key(ts->input_dev, ts->ts_data.prev_button.key_code,
					 BUTTON_RELEASED);

			if (likely(touch_debug_mask_ & (DEBUG_BUTTON | DEBUG_BASE_INFO)))
				TOUCH_INFO_MSG("KEY[%s:%3d] is released\n",
					       get_touch_button_string(ts->ts_data.prev_button.
								       key_code),
...
static int touch_power_cntl(struct lge_touch_data *ts, int onoff)
{
	...
	if (ret >= 0)
		TOUCH_POWER_MSG("power_state[%d]\n", atomic_read(&ts->pdata->curr_pwr_state));
...
static int touch_ic_init(struct lge_touch_data *ts)
{
	...
	if (unlikely(ts->ic_init_err_cnt >= MAX_RETRY_COUNT)) {
		...
		lge_pre_self_diagnosis("i2c", 5, (char *)ts->client->name,
				       (char *)ts->client->driver->driver.name, 5);
#endif
...
/* if (mt_get_gpio_dir(ts_pdata->int_pin)) */
	...
	if (ts_role->ghost_detection_enable) {
		...
		if (ts->gf_ctrl.probe) {
			atomic_set(&ts->gf_ctrl.stage, GHOST_STAGE_1);
			if (touch_drv->ic_ctrl) {
				if (touch_drv->ic_ctrl(ts->client, IC_CTRL_BASELINE, BASELINE_OPEN)
				    < 0) {
					TOUCH_ERR_MSG("IC_CTRL_BASELINE handling fail\n");
					goto err_out_retry;
				}
			...
			if (atomic_read(&ts->gf_ctrl.stage) & GHOST_STAGE_2) {
				atomic_set(&ts->gf_ctrl.stage,
					   GHOST_STAGE_1 | GHOST_STAGE_2 | GHOST_STAGE_4);
				ts->gf_ctrl.ghost_check_count = MAX_GHOST_CHECK_COUNT - 1;
				if (touch_drv->ic_ctrl) {
					if (touch_drv->ic_ctrl(ts->client, IC_CTRL_BASELINE,
							       BASELINE_OPEN) < 0) {
						TOUCH_ERR_MSG("IC_CTRL_BASELINE handling fail\n");
						goto err_out_retry;
					}
				...
				atomic_set(&ts->gf_ctrl.stage, GHOST_STAGE_3);
				if (touch_drv->ic_ctrl) {
					if (touch_drv->ic_ctrl(ts->client, IC_CTRL_BASELINE,
							       BASELINE_FIX) < 0) {
						TOUCH_ERR_MSG("IC_CTRL_BASELINE handling fail\n");
	...
	memset(&ts->ts_data, 0, sizeof(ts->ts_data));
	atomic_set(&ts->fw_info.is_downloading, 0);
...
int ghost_finger_solution(struct lge_touch_data *ts)
{
	...
	if (atomic_read(&ts->gf_ctrl.stage) & GHOST_STAGE_1) {
		if (ts->ts_data.total_num == 0 && ts->ts_data.curr_button.state == 0
			...
			if (ts->gf_ctrl.count < ts->gf_ctrl.min_count
				...
				if (atomic_set(&ts->gf_ctrl.stage) & GHOST_STAGE_2)
					ts->gf_ctrl.ghost_check_count = MAX_GHOST_CHECK_COUNT - 1;
				else
					ts->gf_ctrl.ghost_check_count = 0;
			} else {
			...
			if (ts->gf_ctrl.ghost_check_count >= MAX_GHOST_CHECK_COUNT) {
				...
				atomic_clear_mask(GHOST_STAGE_1, &ts->gf_ctrl.stage);
				if (unlikely(touch_debug_mask_ & DEBUG_GHOST || touch_debug_mask_ & DEBUG_BASE_INFO))
					TOUCH_INFO_MSG("ghost_stage_1: cleared[0x%x]\n", ts->gf_ctrl.stage);
				if (!atomic_read(&ts->gf_ctrl.stage)) {
					if (unlikely(touch_debug_mask_ & DEBUG_GHOST))
			...
			if (unlikely(touch_debug_mask_ & DEBUG_GHOST)
				...
				TOUCH_INFO_MSG
				    ("ghost_stage_1: Not good condition. total[%d] button[%d] id[%d] palm[%d]\n",
				     ts->ts_data.total_num, ts->ts_data.curr_button.state, id,
				     ts->ts_data.palm);
			ts->gf_ctrl.count = ts->gf_ctrl.max_count;
		}
	} else if (atomic_read(&ts->gf_ctrl.stage) & GHOST_STAGE_2) {
		if (ts->ts_data.total_num > 1
		    || (ts->ts_data.total_num == 1 && ts->ts_data.curr_button.state)) {
			/* ts->gf_ctrl.stage |= GHOST_STAGE_1; */
			atomic_set_mask(GHOST_STAGE_1, &ts->gf_ctrl.stage);
			ts->gf_ctrl.ghost_check_count = MAX_GHOST_CHECK_COUNT - 1;
			ts->gf_ctrl.count = 0;
			ts->gf_ctrl.saved_x = -1;
			ts->gf_ctrl.saved_y = -1;
...
/* do nothing */
#else
				if (touch_drv->ic_ctrl
				    (ts->client, IC_CTRL_BASELINE, BASELINE_REBASE) < 0)
					return -1;
#endif
			}
			if (unlikely(touch_debug_mask_ & DEBUG_GHOST))
				TOUCH_INFO_MSG
				    ("ghost_stage_2: multi_finger. return to ghost_stage_1[0x%x]\n",
				     atomic_read(&ts->gf_ctrl.stage));
		}
	} else if (atomic_read(&ts->gf_ctrl.stage) & GHOST_STAGE_3) {
		if (ts->ts_data.total_num == 0 && ts->ts_data.curr_button.state == 0
		    && ts->ts_data.palm == 0) {
			ts->gf_ctrl.ghost_check_count++;

			if (unlikely
			    (touch_debug_mask_ & DEBUG_GHOST
			     || touch_debug_mask_ & DEBUG_BASE_INFO))
			...
			/* ts->gf_ctrl.stage &= ~GHOST_STAGE_3; */
			/* ts->gf_ctrl.stage |= GHOST_STAGE_1; */
			atomic_clear_mask(GHOST_STAGE_3, &ts->gf_ctrl.stage);
			atomic_set_mask(GHOST_STAGE_1, &ts->gf_ctrl.stage);
			ts->gf_ctrl.ghost_check_count = 0;
			ts->gf_ctrl.count = 0;
			ts->gf_ctrl.saved_x = -1;
	...
	} else if (atomic_read(&ts->gf_ctrl.stage) & GHOST_STAGE_4) {
...
#endif
			atomic_set(&ts->gf_ctrl.stage, GHOST_STAGE_CLEAR);
			if (unlikely
			    (touch_debug_mask_ & DEBUG_GHOST
			     || touch_debug_mask_ & DEBUG_BASE_INFO))
...
static void touch_gesture_wakeup_func(struct lge_touch_data *ts)
{
	...
	if (atomic_read(&ts->fw_info.is_downloading) == UNDER_DOWNLOADING) {
...
static void check_log_finger_changed(struct lge_touch_data *ts, u8 total_num)
{
	...
	TOUCH_INFO_MSG("%s %d ts->ts_data.prev_total_num=%d total_num=%d\n", __func__, __LINE__,
		       ts->ts_data.prev_total_num, total_num);
...
static int touch_work_pre_proc(struct lge_touch_data *ts)
{
	...
	if (likely(ts_role->ghost_finger_solution_enable)
	    && unlikely(atomic_read(&ts->gf_ctrl.stage))) {
	...
	if (ts->ts_data.state != TOUCH_BUTTON_LOCK) {
		...
		if (ts->work_sync_err_cnt > 0 && ts->ts_data.prev_button.state == BUTTON_RELEASED) {
			...
			if (ts->ts_data.curr_button.state == BUTTON_PRESSED
				...
				input_report_key(ts->input_dev, ts->ts_data.curr_button.key_code,
						 BUTTON_PRESSED);

				if (likely(touch_debug_mask_ & (DEBUG_BUTTON | DEBUG_BASE_INFO)))
					TOUCH_INFO_MSG("KEY[%s:%3d] is pressed\n",
						       get_touch_button_string(ts->ts_data.
									       curr_button.
									       key_code),
						       ts->ts_data.curr_button.key_code);
...
static int touch_request_firmware(struct lge_touch_data *ts)
{
	...
	ret = request_firmware_mod((const struct firmware **)(&ts->fw_info.fw), fw_name,
				  &ts->client->dev);
	if (ret != 0) {
		ts->fw_info.fw = NULL;
...
/* TOUCH_INFO_MSG("%s e] fw_upgrade_completion : %d\n",__func__, completion_done(&ts->fw_upgrade_completion)); */
	...
	if (MELFAS_binary == NULL || (size_t) sizeof(MELFAS_binary) < 0) {
		...
		TOUCH_INFO_MSG
		    ("MELFAS_binary = 0x%X, (size_t)sizeof(MELFAS_binary) = 0x%X. the data present hex unit",
		     (int)MELFAS_binary, (int)sizeof(MELFAS_binary));
		...
		if ((u16) ts->fw_info.fw->size != MELFAS_binary_nLength) {
			...
			TOUCH_INFO_MSG
			    (" ts->fw_info.fw = 0x%X, ts->fw_info.fw->size = 0x%X. the data present hex unit",
			     (int)ts->fw_info.fw->data, (int)ts->fw_info.fw->size);
		}
...
static void touch_fw_upgrade_func(struct work_struct *work_fw_upgrade)
{
	...
	int saved_state = atomic_read(&ts->pdata->curr_pwr_state);
	...
	atomic_set(&ts->fw_info.is_downloading, UNDER_DOWNLOADING);
...
static int touch_thread_irq_handler(void *dev_id)
{
	...
	do {
		...
		if (unlikely(atomic_read(&ts->device_init) != 1)
		    && atomic_read(&dev_state) == DEV_RESUME) {
			TOUCH_ERR_MSG
			    ("return IRQ_HANDLED becasue of the value that ts->device_init:[%d] dev_state:[%d]\n",
...
static int check_platform_data(struct i2c_client *client)
{
	...
	for (i = 0; i < TOUCH_PWR_NUM; ++i) {
		if (ts_pwr[i].type == 1 && !gpio_is_valid(ts_pwr[i].value)) {
			...
			if (!ts_pwr[i].name[0]) {
				TOUCH_ERR_MSG
				    ("you should assign the supply name for regulator[%d]\n", i);
				return -1;
			}
		}
	}
...
{
	static char *str[10] = {
	"DT_U8", "DT_U16", "DT_U32", "DT_GPIO", "DT_STRING",};
...
static int touch_parse_dt(struct device *dev, struct touch_platform_data *pdata)
{
		...
		for (i = 0; i < caps->number_of_button; i++) {
			...
			if (unlikely(touch_debug_mask_ & DEBUG_CONFIG))
				TOUCH_INFO_MSG("DT entry: button_name[%d] : %s:%d\n", i,
					       get_touch_button_string((u16) caps->button_name[i]),
					       caps->button_name[i]);
		}
	}
	...
	for (i = 0; i < TOUCH_PWR_NUM; ++i) {
		if (pwr[i].type == 1) {
			...
			ret = of_get_named_gpio(np, power_gpio, 0);
			if (ret >= 0) {
				if (unlikely(touch_debug_mask_ & DEBUG_CONFIG))
					TOUCH_INFO_MSG("DT entry[Reparse]: %s = %d (%s)\n",
						       power_gpio, ret,
						       touch_print_dt_type(DT_GPIO));
				pwr[i].value = (int)ret;
			} else {
				pwr[i].value = (int)-1;
			}
	...
	for (itr = map; itr->name; ++itr) {
		...
		case DT_U8:
			...
			*((u8 *) itr->data) = (u8) itr->def_value;
			if (unlikely(touch_debug_mask_ & DEBUG_CONFIG))
				TOUCH_INFO_MSG("DT entry: %s = %d (%s)\n", itr->name,
					       *((u8 *) itr->data), touch_print_dt_type(itr->type));
#endif
			break;
			...
			*((unsigned long *)itr->data) = (unsigned long)itr->def_value;
			if (unlikely(touch_debug_mask_ & DEBUG_CONFIG)) {
				TOUCH_INFO_MSG("DT entry: %s = %Xu (%s)\n", itr->name,
					       itr->def_value, touch_print_dt_type(itr->type));
				TOUCH_INFO_MSG("DT entry: %s = %Xu (%s)\n", itr->name,
					       (itr->def_value & 0x0fffffff),
					       touch_print_dt_type(itr->type));
			}
#endif
			break;
...
static int touch_init_platform_data(struct i2c_client *client)
{
	...
	if (true) {
		...
		ts_caps = devm_kzalloc(&client->dev, sizeof(struct touch_device_caps), GFP_KERNEL);
		if (!ts_caps) {
		...
		ts_limit = devm_kzalloc(&client->dev, sizeof(struct touch_limit_value), GFP_KERNEL);
		if (!ts_limit) {
			dev_err(&client->dev, "Failed to allocate memory : limit\n");
			return -ENOMEM;
		}

		ts_lpwg = devm_kzalloc(&client->dev, sizeof(struct touch_tci_info), GFP_KERNEL);
		if (!ts_lpwg) {
	...
	for (i = 0; i < sc->b_num; i++) {
		if (ts_caps->number_of_button)
			sc->button[i].left =
			    i * (ts_caps->x_max / ts_caps->number_of_button) + sc->b_margin;
		else
		...
		sc->button_cancel[i].right =
		    ...
		    ts_caps->x_max ? sc->button[i].right + (2 * sc->b_margin) : ts_caps->x_max;
...
static int touch_fb_suspend(struct device *device)
{
	...
	if (ts->pdata->role->use_crack_mode) {
		if (atomic_read(&ts->crack_test_state) != CRACK_TEST_FINISH) {
			TOUCH_INFO_MSG
			    ("touch_fb_suspend is not executed - crack check not finished\n");
			return 0;
		}
	}
	...
	ret =
	    wait_for_completion_interruptible_timeout(&ts->irq_completion, msecs_to_jiffies(1000));
	if (ret == 0)
...
static int touch_fb_resume(struct device *device)
{
	...
	if (ts->pdata->lpwg_mode) {
		...
		if (atomic_read(&ts->pdata->curr_pwr_state) == POWER_ON) {
			touch_disable_wake(ts->client->irq);
			safety_reset(ts);
		} else {
...
static ssize_t fw_upgrade_show(struct lge_touch_data *ts, char *buf, bool forceupgrade)
{
	...
	ret +=
	    sprintf(buf + ret, "  1 : Upgrade with the given name in Device Tree [%s]\n",
		    ts_pdata->fw_image);
				...
				bool forceupgrade)
{
	...
	} else if (strncmp(&cmd[strlen(cmd) - 3], ".fw", 3) == 0
		   || strncmp(&cmd[strlen(cmd) - 3], ".FW", 3) == 0) {
	...
	while (atomic_read(&ts->fw_info.is_downloading))
		;

	atomic_set(&ts->fw_info.is_downloading, 1);
	...
	if (ts->pdata->panel_on == POWER_ON || ts->pdata->panel_on == POWER_WAKE) {
		...
		ret =
		    snprintf(buf, PAGE_SIZE,
			     "If you want to check open short, please turn on the LCD.\n");
...
static ssize_t rawdata_show(struct lge_touch_data *ts, char *buf)
{
	...
	if (ts->pdata->panel_on == POWER_ON || ts->pdata->panel_on == POWER_WAKE
		...
		ret =
		    snprintf(buf, PAGE_SIZE,
			     "If you want to check raw data, please turn on the LCD.\n");
		TOUCH_INFO_MSG("If you want to check raw data, please turn on the LCD.\n");
	}
	return ret;
...
static ssize_t delta_show(struct lge_touch_data *ts, char *buf)
{
	...
	if (ts->pdata->panel_on == POWER_ON || ts->pdata->panel_on == POWER_WAKE
		...
		ret =
		    snprintf(buf, PAGE_SIZE,
			     "If you want to check delta, please turn on the LCD.\n");
...
static ssize_t self_diagnostic_show(struct lge_touch_data *ts, char *buf)
{
	...
	if (ts->pdata->panel_on == POWER_ON || ts->pdata->panel_on == POWER_WAKE
		...
		ret =
		    snprintf(buf, PAGE_SIZE,
			     "If you want to check self diagnostic, please turn on the LCD.\n");
...
static ssize_t lpwg_notify_store(struct lge_touch_data *ts, const char *buf, size_t count)
{
	...
	ret = sscanf(buf, "%d %d %d %d %d", &type, &cmd[0], &cmd[1], &cmd[2], &cmd[3]);
	if (ret <= 0)
		return 0;
...
static ssize_t touch_dsv_store(struct lge_touch_data *ts, const char *buf, size_t count)
{
	...
	ret = sscanf(buf, "%d", &ts_pdata->use_dsv);
	if (ret <= 0)
		count = 0;
...
static ssize_t sensor_value_store(struct lge_touch_data *ts, const char *buf, size_t count)
{
	...
	ret = sscanf(buf, "%d", &ts_pdata->sensor_value);
	if (ret <= 0)
		count = 0;
...
static ssize_t enable_sensor_interlock_store(struct lge_touch_data *ts, const char *buf,
					     size_t count)
{
	int ret;
	if (!ts_pdata->panel_on) {
	...
	ret = sscanf(buf, "%d", &ts_pdata->enable_sensor_interlock);
	if (ret <= 0)
		count = 0;
...
static LGE_TOUCH_ATTR(fw_force_upgrade, S_IRUGO | S_IWUSR, fw_force_upgrade_show,
		      fw_force_upgrade_store);
static LGE_TOUCH_ATTR(fw_upgrade, S_IRUGO | S_IWUSR, fw_normal_upgrade_show,
		      fw_normal_upgrade_store);
...
static LGE_TOUCH_ATTR(crack_status, S_IRUGO | S_IWUSR, window_crack_status_show,
		      window_crack_status_store);
...
static LGE_TOUCH_ATTR(enable_sensor_interlock, S_IRUGO | S_IWUSR, enable_sensor_interlock_show,
		      enable_sensor_interlock_store);
#endif
...
static struct attribute *lge_touch_attribute_list[] = {
	&lge_touch_attr_fw_force_upgrade.attr,
	    ...
	    &lge_touch_attr_lpwg_debug.attr, &lge_touch_attr_lpwg_fail_reason.attr,
#if defined(TOUCH_USE_DSV)
	    &lge_touch_attr_dsv.attr,
	    &lge_touch_attr_sensor_value.attr, &lge_touch_attr_enable_sensor_interlock.attr,
#endif
NULL,};
...
.show = lge_touch_attr_show, .store = lge_touch_attr_store,};

static struct kobj_type lge_touch_kobj_type = {
.sysfs_ops = &lge_touch_sysfs_ops, .default_attrs = lge_touch_attribute_list,};
...
static int touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	TOUCH_INFO_MSG("%s-%d], mt_get_gpio_in(ts_pdata->int_pin) : %d\n", __func__, __LINE__,
		       mt_get_gpio_in(ts_pdata->int_pin));
	TOUCH_INFO_MSG("%s-%d],  mt_get_gpio_out(ts_pdata->reset_pin) : %d\n", __func__, __LINE__,
		       mt_get_gpio_out(ts_pdata->reset_pin));
	/* reset pin setting */
#ifndef IS_MTK
	...
	ret = mt_get_gpio_dir(ts_pdata->reset_pin);
	if (ret) {
		if (ret < 0) {
	...
	if (ts_pdata->auto_fw_update && touch_drv->fw_upgrade) {
		...
		ret =
		    wait_for_completion_interruptible_timeout(&ts->fw_upgrade_completion,
	...
	ret = request_threaded_irq(client->irq, touch_irq_handler, touch_thread_irq_handler,
				   IRQF_TRIGGER_LOW | IRQF_ONESHOT | IRQF_NO_SUSPEND, client->name,
				   ts);
...
static struct const dev_pm_ops touch_pm_ops = {
.suspend = touch_suspend, .resume = touch_resume,};
#endif
...
static struct i2c_driver lge_touch_driver = {
	.probe = touch_probe, .remove = touch_remove, .id_table = touch_id_table, .driver = {
		.name = LGE_TOUCH_NAME,
/* .owner = THIS_MODULE, */
/* #if defined(CONFIG_PM) */
/* .pm = &touch_pm_ops, */
...
I2C_BOARD_INFO(TPD_DEV_NAME, 0x34)};

static struct tpd_driver_t tpd_device_driver = {
	.tpd_device_name = TPD_DEV_NAME, .tpd_local_init = touch_local_init,
