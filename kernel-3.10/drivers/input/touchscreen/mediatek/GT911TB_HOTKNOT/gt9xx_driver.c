...
static s32 gtp_init_panel(struct i2c_client *client)
{
...
#else
	{
		...
		if (ret == SUCCESS) {
			...
			drv_cfg_version = send_cfg_buf[sensor_id][0]; /* backupconfig version */
...
/* #ifdef VELOCITY_CUSTOM */
	...
	if (!int_type)
		...
		CUST_EINT_POLARITY_HIGH, tpd_eint_interrupt_handler, 1);
	else
		mt65xx_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_EN,
		CUST_EINT_POLARITY_LOW, tpd_eint_interrupt_handler, 1);
#endif
