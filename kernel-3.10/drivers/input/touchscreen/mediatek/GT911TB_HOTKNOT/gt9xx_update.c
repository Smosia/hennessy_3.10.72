...
#define FIRMWARE_LENGTH (FW_SS51_LENGTH + FW_DSP_LENGTH + FW_BOOT_LENGTH + \
	FW_DSP_ISP_LENGTH + FW_BOOT_ISP_LENGTH + FW_APP_CODE_LENGTH)
...
(4*FW_SECTION_LENGTH+FW_DSP_LENGTH+FW_BOOT_LENGTH+FW_DSP_ISP_LENGTH), FW_BOOT_ISP_LENGTH, SEEK_SET); */
...
buffer = kmalloc(sizeof(u8) * len, GFP_KERNEL);
if (buffer == NULL) {
...
while (length > 0) {
	...
	GTP_DEBUG("load bank%dlength:%dsuccess.", bank, len);
	bank++;
	length -= len;
}
