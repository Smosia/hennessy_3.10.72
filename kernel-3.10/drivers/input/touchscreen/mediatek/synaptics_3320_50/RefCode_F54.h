...
#define TPD_FUN(f)               pr_debug(KERN_ERR "[S3320] %s\n", __func__)
#define TPD_ERR(fmt, args...)    pr_debug(KERN_ERR "[S3320] %s %d : "fmt, __func__, __LINE__, ##args)
#define TPD_LOG(fmt, args...)    pr_debug(fmt, ##args)

#define TOUCH_INFO_MSG(fmt, args...) \
	pr_info("[S3320] [Touch] " fmt, ##args);
...
#define TOUCH_DEBUG_MSG(fmt, args...)			\
	pr_debug("[S3320] [Touch D] [%s %d] " fmt,	\
		__func__, __LINE__, ##args);
