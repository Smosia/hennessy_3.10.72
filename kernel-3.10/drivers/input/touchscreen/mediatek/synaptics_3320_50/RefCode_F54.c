...
#define DO_IF(do_work, goto_error)								\
	do {												\
		if (do_work) {										\
			pr_debug(KERN_INFO "[Touch E] Action Failed [%s %d]\n",	\
				__func__, __LINE__); \
			goto goto_error;				\
		}							\
	} while (0)
		...
		for (i = 0; i < pWB->sheets.count; i++) {
			...
			for (j = 0; j < pWB->sheets.count; j++) {
				...
				for (r = 0; r < TxChannelCount; r++) {
					...
					if (debug_externel_limits) {
						/* printf("FullRawCapacitance L [%d][%d]%f\n",
						   r,c,
						   pWS->rows.row[r+1].cells.cell[2*c+1].d); */
				...
				for (r = 0; r < TxChannelCount; r++) {
					for (c = 0; c < RxChannelCount; c++) {
						row = &pWS->rows.row[r + 1];
						ADCLowerImageLimit[r][c] =
						    (&row->cells.
						     cell[2 * c + 1])->d;
						ADCUpperImageLimit[r][c] =
						    (&row->cells.
						     cell[2 * c + 2])->d;
					}
				}
				...
				for (r = 0; r < TxChannelCount; r++) {
					for (c = 0; c < RxChannelCount; c++) {
						row = &pWS->rows.row[r + 1];
						SensorSpeedLowerImageLimit[r][c] =
						    (&row->cells.
						     cell[2 * c + 1])->d;
						SensorSpeedUpperImageLimit[r][c] =
						    (&row->cells.
						     cell[2 * c + 2])->d;
					}
				}
				...
				for (r = 0; r < TRX_mapping_max; r++) {
					temp = TRxPhysical[r];
					if ((TRxPhysical[r] != 0xFF)
					    && ((&row->cells.cell[temp + 1])->d >
						0)) {
						index = temp / 8;
						offset = 7 - (temp % 8);
						TRX_Open[index] =
						    TRX_Open[index] + pow(2.0,
									  offset);
				...
				for (c = 0; c < 3; c++) {
					row = &pWS->rows.row[r];
					HighResistanceLowerLimit[c] =
					    (float)(&row->cells.cell[2 * c + 1])->d;
					HighResistanceUpperLimit[c] =
					    (float)(&row->cells.cell[2 * c + 2])->d;
				}
						break;
					case 7:	/* MaxMinTest */
						break;
		...
		for (i = 0; i < (int)F12_2DTxCount; i++) {
			...
			if ((ImagepF[i][j] < LowerImageLimit[i][j])
				...
				if (f54_window_crack_check_mode) {
					...
					TOUCH_INFO_MSG("[Touch] Tx [%d] Rx [%d] node_crack_count %d,
		    row_crack_count %d, raw cap %d\n", i, j, node_crack_count, row_crack_count, ImagepF[i][j]);
				} else {
	...
	for (i = 0; i < F12_2DTxCount; i++) {
		for (j = 0; j < F12_2DRxCount; j++) {
			if ((ImagepF[i][j] < NoiseLimitLow) || (ImagepF[i][j] > NoiseLimitHigh)) {
				...
				/* out_buf += sprintf(wlog_buf+out_buf, "Failed Tx [%2d] Rx [%2d]
				   = %3d\n", i, j, ImagepF[i][j]); */
				result = false;
			}
		}
