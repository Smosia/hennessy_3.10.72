...
#define CONVERT_M_DIV           (4)  /* 1/1000 = CONVERT_M */
#define CONVERT_O_DIV           (71)  /* 1/1000 = CONVERT_O */
...
//todo
int lsm6ds3_exist = 0;
extern int bsx_algo_m_enable(int en);
extern int bsx_algo_m_set_delay(u64 ns);
extern int bsx_algo_m_open_report_data(int open);
extern int bsx_algo_m_get_data(int* x ,int* y,int* z, int* status);
extern int bsx_algo_o_enable(int en);
extern int bsx_algo_o_set_delay(u64 ns);
extern int bsx_algo_o_get_data(int* x ,int* y,int* z, int* status);
extern int bsx_algo_o_open_report_data(int open);
...
static int yas_m_get_data(int32_t *x, int32_t *y, int32_t *z, int32_t *status)
{
    ...
    printk("Smosia: yas %i, %i, %i, %i", *x, *y, *z, *status);
    return 0;
}
        ...
        const struct i2c_device_id *id)
{
    ...
    if (raw == NULL) {
        ret = -ENOMEM;
        MAGN_ERR("yl_sensor_debug yas_probe devide raw structure error\n");
        goto error_free_device;
    }
    cal = input_allocate_device();
    if (cal == NULL) {
        ret = -ENOMEM;
        MAGN_ERR("yl_sensor_debug yas_probe devide cal structure error\n");
        goto error_free_device;
    }
    euler = input_allocate_device();
    if (euler == NULL) {
        ret = -ENOMEM;
        MAGN_ERR("yl_sensor_debug yas_probe devide euler structure error\n");
        goto error_free_device;
    }
    ...
    if (ret)
    {
        MAGN_ERR("yl_sensor_debug yas_probe register raw device error\n");
        goto error_free_device;
    }
    ret = input_register_device(cal);
    if (ret)
    {
        MAGN_ERR("yl_sensor_debug yas_probe register cal device error\n");
        goto error_free_device;
    }
    ret = input_register_device(euler);
    if (ret)
    {
        MAGN_ERR("yl_sensor_debug yas_probe register euler device error\n");
        goto error_free_device;
    }
    ...
    if (ret)
    {
        MAGN_ERR("yl_sensor_debug yas_probe create cal sysfs group error\n");
        goto error_free_device;
    }
    ...
    if (ret)
    {
        MAGN_ERR("yl_sensor_debug yas_probe create euler sysfs group error\n");
        goto error_free_device;
    }
...
/*
    ...
    if (ret < 0) {
        ret = -EFAULT;
        MAGN_ERR("yl_sensor_debug yas_probe driver init error\n");
        goto error_remove_sysfs;
    }
    ret = st->mag.init();
    if (ret < 0) {
        ret = -EFAULT;
        MAGN_ERR("yl_sensor_debug yas_probe st->mag.init error\n");
        goto error_remove_sysfs;
    }
    ...
    {
        //ctl.is_use_common_factory = false;
        ctl.m_enable = bsx_algo_m_enable;//yas_m_enable;
        ctl.m_set_delay = bsx_algo_m_set_delay;//yas_m_set_delay;
        ctl.m_open_report_data = bsx_algo_m_open_report_data;//yas_m_open_report_data;
        ctl.o_enable = bsx_algo_o_enable;//yas_o_enable;
        ctl.o_set_delay = bsx_algo_o_set_delay;//yas_o_set_delay;
        ctl.o_open_report_data = bsx_algo_o_open_report_data;//yas_o_open_report_data;
        ctl.is_report_input_direct = false;
        ctl.is_support_batch = st->hw->is_batch_supported;  
        ...
        mag_data.div_m = CONVERT_M_DIV;
        mag_data.div_o = CONVERT_O_DIV;
        mag_data.get_data_o = bsx_algo_o_get_data;//yas_o_get_data;
        mag_data.get_data_m = bsx_algo_m_get_data; //yas_m_get_data;   
    ...
    // {
    //     ctl.is_use_common_factory = false;
    //     ctl.m_enable = lsm6ds3_m_enable;//yas_m_enable;
    //     ctl.m_set_delay = lsm6ds3_m_set_delay;//yas_m_set_delay;
    //     ctl.m_open_report_data = lsm6ds3_m_open_report_data;//yas_m_open_report_data;
    //     ctl.o_enable = lsm6ds3_o_enable;//yas_o_enable;
    //     ctl.o_set_delay = lsm6ds3_o_set_delay;//yas_o_set_delay;
    //     ctl.o_open_report_data = lsm6ds3_o_open_report_data;//yas_o_open_report_data;
    //     ctl.is_report_input_direct = false;
    //     ctl.is_support_batch = st->hw->is_batch_supported;  
    ...
    //     mag_data.div_m = CONVERT_M_DIV;
    //     mag_data.div_o = CONVERT_O_DIV;
    //     mag_data.get_data_o = lsm6ds3_o_get_data;//yas_o_get_data;
    //     mag_data.get_data_m = lsm6ds3_m_get_data; //yas_m_get_data;   
