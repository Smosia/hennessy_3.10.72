...
static int setReg6332(int reg, int gRegV, int gRegM, int gRegSh)
{
    ...
    /*logI("\nQQQS pmic_config_interface %d %d %d %d (0x%x 0x%x 0x%x 0x%x)\n",reg,
	 gRegV, gRegM, gRegSh, reg, gRegV, gRegM, gRegSh);*/
    return 0;
}
/*
...
static int constant_flashlight_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	/*PK_DBG("constant_flashlight_ioctl() line=%d cmd=%d, ior=%d, iow=%d iowr=%d
	arg=%lu\n",__LINE__, cmd, ior, iow, iowr, arg);*/
	/*PK_DBG("constant_flashlight_ioctl() line=%d cmd-ior=%d, cmd-iow=%d cmd-iowr=%d
