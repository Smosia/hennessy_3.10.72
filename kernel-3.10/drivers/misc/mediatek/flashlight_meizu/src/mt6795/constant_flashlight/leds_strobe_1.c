...
#define TAG_NAME "[leds_strobe_1.c]"
...
#define I2C_STROBE_MAIN_CHANNEL_1             1
			...
			const struct i2c_device_id *id)
{
	...
	printk("LM3643 IC1 Initializing is done -\n");
    return 0;

err_chip_init:
	i2c_set_clientdata(client, NULL);
	kfree(chip);
	printk("LM3643_IC1 probe is failed \n");
	return -ENODEV;
}
...
#define LM3643_NAME "leds-LM3643-IC1"
...
{
	i2c_register_board_info(I2C_STROBE_MAIN_CHANNEL_1, &i2c_LM3643, 1);
...
int Flash_Setduty_IC1(unsigned char led_num,int duty,bool mode)
{
 ...
 if(led_num == COOL_LED){  //set duty for cool led
    ...
    PK_DBG("Flash_Setduty led_num:%d,duty:%d,mode:%d,Value:%d\n",led_num,duty,mode,buf[1]);
    return 0;
}
