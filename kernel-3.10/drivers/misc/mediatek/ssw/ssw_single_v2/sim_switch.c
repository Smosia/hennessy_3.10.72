...
static ssize_t ssw_attr_show(struct kobject *kobj, struct attribute *attr, char *buffer);
static ssize_t ssw_attr_store(struct kobject *kobj, struct attribute *attr, const char *buffer, size_t size);
static ssize_t ssw_mode_show(struct kobject *kobj, char *page);
static ssize_t ssw_mode_store(struct kobject *kobj, const char *page, size_t size);
...
static ssize_t ssw_mode_store(struct kobject *kobj, const char *buffer, size_t size)
{
	unsigned int mode;
...
static int set_sim_gpio(unsigned int mode)
{
	...
	SSW_DBG("mode(%d),SIM1(eint=%d, sclk=%d, srst=%d , sio=%d) SIM2(eint=%d,sclk=%d, srst=%d , sio=%d)\n", mode,
		mt_get_gpio_mode(GPIO_SIM1_HOT_PLUG), mt_get_gpio_mode(GPIO_SIM1_SCLK),
		mt_get_gpio_mode(GPIO_SIM1_SRST), mt_get_gpio_mode(GPIO_SIM1_SIO),
		mt_get_gpio_mode(GPIO_SIM2_HOT_PLUG), mt_get_gpio_mode(GPIO_SIM2_SCLK),
		mt_get_gpio_mode(GPIO_SIM2_SRST), mt_get_gpio_mode(GPIO_SIM2_SIO));
#else
...
static int sim_switch_init(void)
{
	...
	SSW_DBG("SIM1 sclk= en=%d,dir=%d,in=%d,out=%d\n",
		mt_get_gpio_pull_enable(GPIO_SIM1_SCLK), mt_get_gpio_dir(GPIO_SIM1_SCLK),
		mt_get_gpio_in(GPIO_SIM1_SCLK), mt_get_gpio_out(GPIO_SIM1_SCLK));
	SSW_DBG("SIM1 srst= en=%d,dir=%d,in=%d,out=%d\n",
		mt_get_gpio_pull_enable(GPIO_SIM1_SRST), mt_get_gpio_dir(GPIO_SIM1_SRST),
		mt_get_gpio_in(GPIO_SIM1_SRST), mt_get_gpio_out(GPIO_SIM1_SRST));
	SSW_DBG("SIM1 sio= en=%d,dir=%d,in=%d,out=%d\n",
		mt_get_gpio_pull_enable(GPIO_SIM1_SIO), mt_get_gpio_dir(GPIO_SIM1_SIO),
		mt_get_gpio_in(GPIO_SIM1_SIO), mt_get_gpio_out(GPIO_SIM1_SIO));

	SSW_DBG("SIM2 sclk= en=%d,dir=%d,in=%d,out=%d\n",
		mt_get_gpio_pull_enable(GPIO_SIM2_SCLK), mt_get_gpio_dir(GPIO_SIM2_SCLK),
		mt_get_gpio_in(GPIO_SIM2_SCLK), mt_get_gpio_out(GPIO_SIM2_SCLK));
	SSW_DBG("SIM2 srst= en=%d,dir=%d,in=%d,out=%d\n",
		mt_get_gpio_pull_enable(GPIO_SIM2_SRST), mt_get_gpio_dir(GPIO_SIM2_SRST),
		mt_get_gpio_in(GPIO_SIM2_SRST), mt_get_gpio_out(GPIO_SIM2_SRST));
	SSW_DBG("SIM2 sio= en=%d,dir=%d,in=%d,out=%d\n",
		mt_get_gpio_pull_enable(GPIO_SIM2_SIO), mt_get_gpio_dir(GPIO_SIM2_SIO),
		mt_get_gpio_in(GPIO_SIM2_SIO), mt_get_gpio_out(GPIO_SIM2_SIO));
#endif
