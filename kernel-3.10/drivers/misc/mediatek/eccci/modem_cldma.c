...
static void md_cd_ccif_work(struct work_struct *work)
{
    ...
    struct ccci_modem *md = md_ctrl->txq[0].modem;
    CCCI_INF_MSG(md->index, TAG, "Smosia: md_cd_ccif_work channel_id=%d start\n", md_ctrl->channel_id);
...
static irqreturn_t md_cd_ccif_isr(int irq, void *data)
{
    struct ccci_modem *md = (struct ccci_modem *)data;
    struct md_cd_ctrl *md_ctrl = (struct md_cd_ctrl *)md->private_data;
    CCCI_INF_MSG(md->index, TAG, "Smosia: md_cd_ccif_isr start\n");
    ...
    CCCI_INF_MSG(md->index, TAG, "Smosia: md_cd_ccif_isr channel_id=%d\n", md_ctrl->channel_id);
...
#else
    CCCI_INF_MSG(md->index, TAG, "Smosia: md_cd_ccif_isr:md_cd_ccif_work start\n");
    md_cd_ccif_work(&md_ctrl->ccif_work);
    CCCI_INF_MSG(md->index, TAG, "Smosia: md_cd_ccif_isr end\n");
#endif
    return IRQ_HANDLED;
}
...
static int md_cd_send_runtime_data(struct ccci_modem *md, unsigned int sbp_code)
{
    ...
    if(is_meta_mode())
        CCCI_ERR_MSG(md->index, TAG, "Smosia: meta mode");
    if(is_advanced_meta_mode())
        CCCI_ERR_MSG(md->index, TAG, "Smosia: advanced meta mode");
...
static int ccci_modem_probe(struct platform_device *plat_dev)
{
    ...
    md_ctrl->rgpd_dmapool = dma_pool_create("CLDMA_RGPD_DMA",
                        ...
                        16,
                        0);
    CCCI_INF_MSG(md->index, TAG, "Smosia: ccci_modem_probe:md_cd_ccif_work start\n");
    INIT_WORK(&md_ctrl->ccif_work, md_cd_ccif_work);
    CCCI_INF_MSG(md->index, TAG, "Smosia: ccci_modem_probe:md_cd_ccif_work end\n");
