...
* Return      :  0, reading block setting err
                 1, reading block setting ok 
**************************************************************************************************/
bool s5k5e8_check_otp_group_flag(u8 group)
{
	u8 val = 0;
	int i;
    LOG_INF("s5k5e8_check_otp_group group = 0x%02x.\n",group);
	
	s5k5e8_start_read_otp(4);

	if(group == 2)
	{
		val = read_cmos_sensor(0x0A33);
...
int s5k5e8_read_otp_wb(struct s5k5e8_otp_struct *otp)
{
	...
	u8 group1_flag = 0;	
	u8 group_id = 0;	
	U16 basic_address = 0x0A04;
	U16 temp_address = 0x0A04;
	
	group2_flag = s5k5e8_check_otp_group_flag(2);
	...
	{
		group1_flag = s5k5e8_check_otp_group_flag(1);
    ...
    LOG_INF("s5k5e8 otp group_id = 0x%02x.\n",group_id);

	if(group_id != 0)
	{
		s5k5e8_start_read_otp(4);

		temp_address = basic_address + (group_id - 1)*24;
	    ...
	    otp->did = read_cmos_sensor(temp_address++);
...
void s5k5e8_algorithm_otp_wb1(struct s5k5e8_otp_struct *otp)
{
	...
	golden_r = (otp->g_r_h << 8)|(otp->g_r_l);
	golden_g = (otp->g_g_h << 8)|(otp->g_g_l);
	golden_b = (otp->g_b_h << 8)|(otp->g_b_l);

    LOG_INF("unit_r=0x%x, unit_g=0x%x, unit_b=0x%x, golden_r=0x%x, golden_g=0x%x, golden_b=0x%x, \n"
            ,unit_r
            ,unit_g
            ,unit_b
            ,golden_r
	...
	R_to_G=(unit_r*512/unit_g);
	B_to_G=(unit_b*512/unit_g);
	G_to_G = 0x200;	

	Golden_RG=(golden_r*512/golden_g);
	Golden_BG=(golden_b*512/golden_g);
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 150){
		...
		write_cmos_sensor(0x0340,0x0F); //20150713
		write_cmos_sensor(0x0341,0x60); //20150713
	...
	//Delay 1 frame 
	  mDELAY(42); 
		...
		write_cmos_sensor(0x0385,0x01);
		write_cmos_sensor(0x0387,0x01);
		write_cmos_sensor(0x0340,0x09);         //20150713
		write_cmos_sensor(0x0341,0x9C);         //20150713
		write_cmos_sensor(0x0342,0x0B);
		write_cmos_sensor(0x0343,0x28);
		write_cmos_sensor(0x0200,0x00);
		write_cmos_sensor(0x0201,0x00);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {	
				...
				if(s5k5e8_get_module_id()==7)
				{
					Sub_Camera_MID = 7;// 
					LOG_INF("OFILM 5E8 module!!!!\n");
	  				*sensor_id = S5K5E8YX_SENSOR_ID;
					return ERROR_NONE;
				}
				else
				{
					LOG_INF("NOT OFILM 5E8 module!!!!\n");
