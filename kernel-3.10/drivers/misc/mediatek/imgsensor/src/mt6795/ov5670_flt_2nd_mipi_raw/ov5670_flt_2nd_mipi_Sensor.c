...
#endif
#define PFX "ov5670_flt_2nd_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV5670_FLT_2ND_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
{
	printk("pxs_ov5670_flt_2ND_otp ++++++++++++++++  %s \n", __func__); //pei_add
	...
	OV5670_OTP_read_end();
	if (flag == 0x00) {
		printk("pxs_ov5670_flt_2ND_otp check_otp_wb retun 0,group index is empty; 		  %s \n",__func__); //pei_add
		return 0;
	}
	else if (flag & 0x02) {
		printk("pxs_ov5670_flt_2ND_otp check_otp_wb return 1,group index has invalid data; %s \n", __func__); //pei_add
		return 1;
	}
	else {
		printk("pxs_ov5670_flt_2ND_otp check_otp_wb return 2 ,group index has valid data ; %s \n", __func__); //pei_add
		return 2;
	}
}
...
static int check_otp_info(int index)
{
	...
	OV5670_OTP_read_end();
	if (flag == 0x00) {
		printk("pxs_ov5670_flt_2ND_otp check_otp_info retun 0,group index is empty; 		  %s \n",__func__); //pei_add
		return 0;
	}
	else if (flag & 0x02) {
		printk("pxs_ov5670_flt_2ND_otp check_otp_info return 1,group index has invalid data; %s \n", __func__); //pei_add
		return 1;
	}
	else {
		printk("pxs_ov5670_flt_2ND_otp check_otp_info return 2 ,group index has valid data ; %s \n", __func__); //pei_add
		return 2;
	}
}
...
static int read_otp_wb(int index, struct otp_struct *otp_ptr)
{
	...
	printk("pxs_ov5670_flt_2ND_otp rg_ratio =%x,bg_ratio=%x, %s \n",(*otp_ptr).rg_ratio ,(*otp_ptr).bg_ratio, __func__); //pei_add
...
static int update_awb_gain(int R_gain, int G_gain, int B_gain)
{
	...
	printk("pxs_ov5670_flt_2ND_otp2 AA11=%x ,AA12=%x ,AA13=%x ,AA14=%x ,AA15=%x ,AA16=%x \n ", AA11,AA12,AA13,AA14,AA15,AA16);
	return 0;
}
...
static int update_otp_wb()
{
	...
	if (i>3) {
		...
		printk("pxs_ov5670_flt_2ND_otp	no valid wb OTP data  %s \n", __func__); //pei_add
		return 1;
	}
	...
	if (i>3) {
		// no valid wb OTP data
		printk("pxs_ov5670_flt_2ND_otp	no valid info OTP data  %s \n", __func__); //pei_add
		return 1;
	}
	printk("pxs_ov5670_flt_2ND_otp	otp_index=%d, otp_info_index=%d %s \n", otp_index, info_index, __func__); //pei_add

	read_otp_info(info_index, &current_otp);
	if (current_otp.lens_id == OV5670_LENS_ID) {
		if (current_otp.module_integrator_id == 1) {
			RG_Ratio_Typical = 331;
			BG_Ratio_Typical = 316;
		}
		...
		printk("pxs_ov5670_otp current_otp.lens_id != %d: %d %s \n", OV5670_LENS_ID, current_otp.lens_id, __func__);
		return 2;
	}
	...
	printk("pxs_ov5670_flt_2ND_otp	rg=%x,bg=%x,  %s \n",rg,bg, __func__); //pei_add
	printk("pxs_ov5670_flt_2ND_otp  nR_G_gain=%x, nB_G_gain=%x, nG_G_gain=%x,  %s \n",nR_G_gain,nB_G_gain,nG_G_gain, __func__); //pei_add
	...
	printk("pxs_ov5670_flt_2ND_otp  R_gain=%x, G_gain=%x, B_gain=%x,  %s \n",R_gain ,B_gain,G_gain, __func__); //pei_add
...
{
	LOG_INF("OV5670_flt_2ND_Sensor_Init_2lane E\n");
...
{
	LOG_INF("OV5670_flt_2ND_PreviewSetting_2lane enter\n");
...
{
	LOG_INF("OV5670_flt_2ND_CaptureSetting_2lane enter! currefps:%d\n",currefps);
		...
		do
		{
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == OV5670_SENSOR_ID)
			{
				sensor_init();
				if (current_otp.lens_id == OV5670_LENS_ID &&
					(current_otp.module_integrator_id == 1 || current_otp.module_integrator_id == 7))
				{
					*sensor_id == OV5670_FLT_2ND_SENSOR_ID;
...
static kal_uint32 open(void)
{
	...
	if (current_otp.lens_id != OV5670_LENS_ID ||
		current_otp.module_integrator_id != 1 ||
		current_otp.module_integrator_id != 7)
		return ERROR_SENSOR_CONNECT_FAIL;
