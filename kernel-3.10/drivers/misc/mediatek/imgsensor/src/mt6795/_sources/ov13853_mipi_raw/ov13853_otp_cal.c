...
static unsigned char OV13853MIPI_WRITE_ID = 0x20; //0x6C
static unsigned char EEPROM_WRITE_ID = 0XA0;
...
kal_uint16 OV13853_read_eeprom(kal_uint32 addr)
{
    kal_uint16 get_byte=0;
...
//  apply_otp(&current_otp);
    read_eeprom(&current_otp);
    apply_eeprom(&current_otp);
}
...
int read_eeprom(struct otp_struct *otp_ptr)
{
    int otp_flag = 0;
    int addr = 0;
    int temp = 0;
    ...
    // valid info and awb in eeprom
    (*otp_ptr).flag = 0xC0; // valid info and AWB in OTP
    (*otp_ptr).module_integrator_id = OV13853_read_eeprom(0x0004); // MI ID
    (*otp_ptr).lens_id          = OV13853_read_eeprom(0x0005);     // Lens ID
    (*otp_ptr).production_year  = OV13853_read_eeprom(0x0001);     // Year
    (*otp_ptr).production_month = OV13853_read_eeprom(0x0002);     // Month
    (*otp_ptr).production_day   = OV13853_read_eeprom(0x0003);     // Day

    //temp = OV13853_read_eeprom(0x0021); //AWB_RG_LSB
    (*otp_ptr).rg_ratio = ((OV13853_read_eeprom(0x0021)&0x03)<<8) | OV13853_read_eeprom(0x0022);

    //temp = OV13853_read_eeprom(0x0023); //AWB_BG_LSB
    (*otp_ptr).bg_ratio = ((OV13853_read_eeprom(0x0023)&0x03)<<8) | OV13853_read_eeprom(0x0024);
	printk("rg_ratio = 0x%x, bg_ratio = 0x%x",(*otp_ptr).rg_ratio,(*otp_ptr).bg_ratio);
    //RG_Ratio_Typical = 290;
    //BG_Ratio_Typical = 295;   
    //temp = OV13853_read_eeprom(0x27); //Golden_RG_LSB
    //RG_Ratio_Typical = (OV13853_read_eeprom(0x28) << 2) + temp;

    //temp = OV13853_read_eeprom(0x29); //Golden_BG_LSB
    //BG_Ratio_Typical = (OV13853_read_eeprom(0x2A) << 2) + temp;

    // EEPROM Lenc Calibration
/*    for (i = 0; i < 360; i++)
    {
        (* otp_ptr).lenc[i] = OV13853_read_eeprom(0x0041 + i);
	checksumLSC += (* otp_ptr).lenc[i];
//	LOG_INF(" Lenc (* otp_ptr).lenc[%d] : %x \n", i, (* otp_ptr).lenc[i]);
    }
    checksumLSC = (checksumLSC) % 255 + 1;
    temp = OV13853_read_eeprom(0x01A9);
...
int apply_eeprom(struct otp_struct *otp_ptr)
{
    int rg = 0;
    int bg = 0;
    int R_gain = 0;
...
//      LOG_INF(" apply OTP Lenc Calibration : %x \n", (*otp_ptr).flag);
        ...
        for (i = 0; i < 360; i++)
        {
            ...
            OV13853_R2A_write_i2c(0x5200 + i, (*otp_ptr).lenc[i]);
        }
    }
#endif
    return 0;
