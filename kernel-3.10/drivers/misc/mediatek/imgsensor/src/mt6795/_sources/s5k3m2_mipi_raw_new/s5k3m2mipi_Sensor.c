...
static imgsensor_info_struct imgsensor_info = { 
	...
	//liuying@wind-mobi.com 20150409 begin 20150409 for mms cannot video
	.normal_video = { 
		.pclk = 440000000,
		.linelength = 4592,
		.framelength = 3188,
	...
	//liuying@wind-mobi.com 20150409 end 20150409 for mms cannot video
	.hs_video = {
		.pclk = 440000000,
		.linelength = 4592,
		.framelength = 798,
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] =	 
{
 ...
 { 4192, 3104,	  0,  0, 4192, 3104, 2096,  1552, 0000, 0000, 2096, 1552, 0,	0, 2096,  1552}, // video  //liuying 20150409 for mms cannot video
...
//liuying@wind-mobi.com 20150425 begin
#if 0
...
#endif
//liuying@wind-mobi.com 20150425 end
 ...
 **********************************/
#define S5K3m2_USE_LSC_OTP_BB
#ifdef S5K3m2_USE_LSC_OTP_BB//#ifdef CONFIG_MTK_CAM_CAL
extern int read_3m2_eeprom_for_mtk(void);
extern int read_3m2_eeprom_for_mtk_new(void);
#endif
...
void S5K3m2_MIPI_read_otp_wb(struct S5K3m2_MIPI_otp_struct *otp)
{
  	...
  	//if(FLG==0)
		//S5K3m2DB("No OTP Data or OTP data is invalid");
   	//else
   	{
		golden_R = read_3m2_eeprom_reg(0x0010);           	 // read Golden value
		golden_Gr= read_3m2_eeprom_reg(0x0011);              // read Golden value
 		golden_Gb= read_3m2_eeprom_reg(0x0012);              // read Golden value
		golden_B = read_3m2_eeprom_reg(0x0013);              // read Golden value
  		...
  		current_R =  read_3m2_eeprom_reg(0x000c);
  		current_Gr = read_3m2_eeprom_reg(0x000d);
  		current_Gb = read_3m2_eeprom_reg(0x000e);
  		current_B =  read_3m2_eeprom_reg(0x000f); 
...
void S5K3m2_MIPI_write_otp_wb(struct S5K3m2_MIPI_otp_struct *otp)
{
   ...
   write_cmos_sensor_8(0x020e, (G_GAIN>>8)); //g_gain
   write_cmos_sensor_8(0x020f, (G_GAIN&0xff)); // g_gain
   write_cmos_sensor_8(0x0210, (R_GAIN>>8)); // r_gain
   write_cmos_sensor_8(0x0211, (R_GAIN&0xff)); // r_gain
   write_cmos_sensor_8(0x0212, (B_GAIN>>8)); // b_gain
   write_cmos_sensor_8(0x0213, (B_GAIN&0xff)); // b_gain 
   write_cmos_sensor_8(0x0214, (G_GAIN>>8)); // g_gain
   write_cmos_sensor_8(0x0215, (G_GAIN&0xff)); // g_gain 
}
...
//                otp_update();
				//read_3m2_eeprom(0x0766, s5k3m2_eeprom_data, 1404);//liuying 20150210 test
				//liuying@wind-mobi.com 20150314 add begin
				#ifdef S5K3m2_USE_LSC_OTP_BB
					read_3m2_eeprom_for_mtk_new();
					LOG_INF("liuying read 3m2 eeprom for mtk done\n");    
				#endif
				//liuying@wind-mobi.com 20150314 add end
                return ERROR_NONE;
            }   
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	switch (scenario_id) {
		...
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			...
			preview(image_window, sensor_config_data);//liuying@wind-mobi.com 20150409 for mms cannot video
			break;	  
		case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        // add by liuying@wind-mobi.com 20150605 begin
		/*case SENSOR_FEATURE_GET_SENSOR_PDAF_EEPROM_DATASIZE:	 
			...
			*feature_para_len=4;
			break;	 */
		// add by liuying@wind-mobi.com 20150605 end
