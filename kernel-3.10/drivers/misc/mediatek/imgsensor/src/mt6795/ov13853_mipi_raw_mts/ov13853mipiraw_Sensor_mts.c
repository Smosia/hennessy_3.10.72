...
extern int OV13853_selective_read_eeprom_mts_VCM_ID(kal_uint16 addr);

static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV13853_SENSOR_ID_MTS,		//record sensor id defined in Kd_imgsensor.h
...
write_cmos_sensor(0x510d, 0xfe);// neil add 20151122
//write_cmos_sensor(0x510e, 0x00);// neil add 20151122
...
if (capture_first_flag == 0)
{
	...
	//	write_cmos_sensor(0x3836, 0x04); //
		...
		write_cmos_sensor(0x5401, 0x71); //
		write_cmos_sensor(0x5405, 0x80); //
	//	write_cmos_sensor(0x5b21, 0x6e); // 91
	//	write_cmos_sensor(0x5b22, 0x01); // 02
	//	write_cmos_sensor(0x5b23, 0x6e); // 91
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	VCM_Id = OV13853_selective_read_eeprom_mts_VCM_ID(0x0006);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == OV13853_SENSOR_ID) {
				if (VCM_Id == 1)
					*sensor_id = OV13853_SENSOR_ID_MTS;
				else
				{
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
		  //add for ov13853 pdaf
			read_otp_pdaf_data_mts((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			//LOG_INF("OV13853_read_eeprom =  we use preview scenario \n");
			break;
