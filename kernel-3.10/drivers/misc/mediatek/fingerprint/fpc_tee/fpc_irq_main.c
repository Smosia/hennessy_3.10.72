...
static int fpc_irq_init(void)
{
	...
	struct platform_device_info fpc_irq_dev_info = { .name = FPC_IRQ_DEV_NAME, };
	fpc_irq_platform_device = platform_device_register_full(&fpc_irq_dev_info);
				...
				fpc_irq_pdata_t *pdata)
{
	...
	// mt_set_gpio_mode(pdata->irq_gpio, 4);
	// mt_set_gpio_dir(pdata->irq_gpio, 0);
	// mt_set_gpio_pull_enable(pdata->irq_gpio, 0);
	// mt_eint_set_sens(pdata->irq_gpio, 0);
	// mt_eint_set_polarity(pdata->irq_gpio, 1);
	// mt_eint_set_hw_debounce(pdata->irq_gpio, 1);
	// mt_eint_registration(pdata->irq_no, 1, fpc_irq_interrupt);
	// request_threaded_irq(pdata->irq_no, fpc_irq_interrupt, 0, 1, "fpc_irq", &fpc_irq_data->pdata);
	// mt_eint_mask(pdata->irq_no);

	mt_set_gpio_out(0x80000000|pdata->rst_gpio, 1);
	udelay(10);
	mt_set_gpio_out(0x80000000|pdata->rst_gpio, 0);
	udelay(500); // = hex/A7C6
	mt_set_gpio_out(0x80000000|pdata->rst_gpio, 1);
	udelay(10);
	mt_set_gpio_out(0x80000000|pdata->rst_gpio, 1);
				...
				fpc_irq_pdata_t *pdata)
{
	...
	request_threaded_irq(pdata->irq_no, fpc_irq_interrupt, NULL,
				IRQF_TRIGGER_RISING, FPC_IRQ_DEV_NAME, pdata);
