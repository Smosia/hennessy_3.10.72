...
{
	push_table(lcm_initialization_setting_ext,
		   sizeof(lcm_initialization_setting_ext) / sizeof(struct LCM_setting_table), 1);
...
{
#if defined(CONFIG_LEDS_LM3632)
    /* temporary remove : check DSV control */
	/* chargepump_DSV_off(); */
#else
	mt_set_gpio_mode(GPIO_DSV_AVDD_EN, GPIO_DSV_AVDD_EN_M_GPIO);
	mt_set_gpio_pull_enable(GPIO_DSV_AVDD_EN, GPIO_PULL_ENABLE);
